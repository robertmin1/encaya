...
STAPLED_TEST_TMPDIR="$TEST_TMPDIR/stapled"
...
function start_https_server()
{
    ...
    write_https_docroot "$HTTPS_DOCROOT" "Cool or nah"

    (
        cd "$HTTPS_DOCROOT"
        openssl s_server -accept "$HTTPS_SERVER_PORT" -cert "$https_server_cert" -key "$https_server_key" -WWW
    ...
    if ! kill -0 "$HTTPS_SERVER_PID" 2>/dev/null; then
        ...
        fail_test "$start_err_msg"
    fi
}

function start_stapled_https_server()
{
    pkill -f "openssl s_server -accept $STAPLED_HTTPS_SERVER_PORT" 2>/dev/null || true
    write_https_docroot "$STAPLED_HTTPS_DOCROOT" "Cool or nah stapled"
    ...
    if ! kill -0 "$STAPLED_HTTPS_SERVER_PID" 2>/dev/null; then
        ...
        fail_test "Local stapled HTTPS server failed to start"
    fi
}
...
start_https_server "$LEAF_CERT" "$LEAF_KEY" "Local HTTPS server failed to start"
...
chromium_output=$(chromium_fetch_dom_impl "$CHROME_PROFILE_DIR" "$TEST_TMPDIR/chrome.log" "$HASHED_DOMAIN" "https://$HASHED_DOMAIN:$HTTPS_SERVER_PORT/index.html" || true)
...
start_https_server "$EXPIRED_LEAF_CERT" "$EXPIRED_LEAF_KEY" "Expired-cert HTTPS server failed to start"
...
function generate_stapled_pubkey_material()
{
...
function build_stapled_message()
{
...
function generate_stapled_leaf_cert()
{
...
generate_stapled_pubkey_material

echo "Pre-register $STAPLED_DOMAIN"
stapled_name_new_output=$($bitcoin_cli name_new "$STAPLED_NAME")
stapled_name_new_txid=$(echo "$stapled_name_new_output" | jq -r '.[0]')
stapled_name_new_rand=$(echo "$stapled_name_new_output" | jq -r '.[1]')
...
STAPLED_BLOCKCHAIN_ADDRESS=$($bitcoin_cli name_show "$STAPLED_NAME" | jq -r '.address')
...
assert_contains "$stapled_negative_output" "404" "Stapled AIA missing-signature check did not return 404"
...
assert_contains "$stapled_wrong_sig_output" "404" "Stapled AIA wrong-signature check did not return 404"
...
assert_contains "$stapled_multi_ca_pem" "BEGIN CERTIFICATE" "Stapled AIA multi-signature acceptance failed"
...
assert_contains "$stapled_ca_pem" "BEGIN CERTIFICATE" "Encaya did not return stapled $STAPLED_DOMAIN Domain AIA Parent CA"
...
stapled_chromium_output=$(chromium_fetch_dom_impl "$STAPLED_CHROME_PROFILE_DIR" "$STAPLED_TEST_TMPDIR/chrome.log" "$STAPLED_DOMAIN" "https://$STAPLED_DOMAIN:$STAPLED_HTTPS_SERVER_PORT/index.html" || true)
assert_contains "$stapled_chromium_output" "Cool or nah stapled" "Chromium did not render expected page content over stapled TLS validation"

if [[ "$stapled_chromium_output" == *"Your connection is not private"* ]]; then
    fail_test "Chromium reported certificate error instead of successful stapled validation"
fi
